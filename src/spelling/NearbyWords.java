...
public class NearbyWords implements SpellingSuggest {
		...
		for(int index = 0; index <= s.length(); index++){
			...
			String newWord = "";
				...
				for(int position = 0; position < s.length(); position++){
					if( index > position ){
						begin += s.charAt(position);
					}else{				
						end += s.charAt(position);
						}
				}
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++) {
			
				newWord = begin + Character.toString ((char) charCode) + end;
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		while( retList.size() < numSuggestions && THRESHOLD > counter ){

			current = distanceOne( ((LinkedList<String>) queue).poll(), true);
			...
			retList.add(((LinkedList<String>) queue).peek());
